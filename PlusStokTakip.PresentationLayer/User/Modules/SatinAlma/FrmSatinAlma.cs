...
namespace PlusStokTakip.PresentationLayer.User.Modules.SatinAlma
{
    public partial class FrmSatinAlma : DevExpress.XtraEditors.XtraForm
    {
        ...
        {
            var products = _productsManager.TGetAll().Where(pr => pr.IsActive == true).Select(pr => new
            {
                pr.ProductID,
                pr.ProductName,
                pr.Barcode
            ...
            var purchases = _purchaseManager.TGetAll().Where(pu => pu.IsActive == true).Select(pu => new
            {
                pu.PurchaseID,
