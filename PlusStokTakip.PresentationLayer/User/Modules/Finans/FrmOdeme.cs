...
namespace PlusStokTakip.PresentationLayer.User.Modules.Finans
{
    public partial class FrmOdeme : DevExpress.XtraEditors.XtraForm
    {
        ...
        {
            var customers = _customersManager.TGetAll().Where(c=>c.IsActive==true).Select(c => new { c.CustomerID, c.CustomerName });
            var suppliers = _suppliersManager.TGetAll().Where(s => s.IsActive == true).Select(s => new { s.SupplierID, s.SupplierName });
            var payments = _paymentsManager.TGetAll().Where(p => p.IsActive == true).Select(p => new
            {
                p.PaymentID,
        ...
        {
            lkpMusteri.Properties.DataSource = _customersManager.TGetAll().Where(c=>c.IsActive==true).Select(c => new
            {
                c.CustomerID,
                c.CustomerName,
                c.CompanyName,
        ...
        {
            lkpTedarikci.Properties.DataSource = _suppliersManager.TGetAll().Where(s=>s.IsActive==true).Select(s => new
            {
                s.SupplierID,
                s.SupplierName,
                s.ContactPerson,
